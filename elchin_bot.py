...
# === Gemini AI Integration with character limit instruction ===
@bot.tree.command(name="ask", description="Ask Gemini AI something")
async def ask(interaction: discord.Interaction, prompt: str):
    await interaction.response.defer()  # show "thinking"
    try:
        # Tell Gemini to limit the response to 1000 characters
        gemini_prompt = f"Answer the following in less than 1000 characters:\n{prompt}"
        ...
        # Truncate to 1024 chars for Discord embed field
        truncated_response = response.text[:1024]

        embed = discord.Embed(
            title="🤖 Gemini AI Response",
            color=discord.Color.blurple()
        )
        embed.add_field(name="Your Question", value=prompt[:1024], inline=False)
        embed.add_field(name="Gemini Says", value=truncated_response, inline=False)
